...
- Add ``modelopt.onnx.quantization.sensitivity`` — per-op-type or per-node accuracy sensitivity ranking for ONNX PTQ, plus coverage or threshold-based exclusion picker that turns the ranking into an actionable ``--nodes_to_exclude`` list.
